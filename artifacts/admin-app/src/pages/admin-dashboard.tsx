...
import {
  ...
  AlertTriangle, CheckCircle2, Sparkles, Inbox, Loader2, Receipt, LogOut,
} from 'lucide-react'
...
type TabId = 'overview' | 'users' | 'payments' | 'feedback' | 'subscriptions' | 'gmail' | 'activity' | 'scan-logs' | 'receipts' | 'warranties'
...
export default function AdminDashboard() {
  ...
  const { data: gmailData, isLoading: loadingGmail } = useAdminGmailAccounts(gmailSearch)
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-card/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-sm">ReceiptGuard Admin</span>
            <Badge variant="destructive" className="text-[10px] px-1.5 py-0">Admin Only</Badge>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground hidden sm:block">{user?.email}</span>
            <Button size="sm" variant="ghost" onClick={handleSignOut} className="h-7 gap-1.5 text-xs">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">System-wide metrics and user management</p>
        </div>
                ...
                <Card key={label}>
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="p-1 bg-secondary rounded"><Icon className="w-3 h-3" /></div>
                    </div>
        ...
        {tab === 'payments' && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><DollarSign className="w-5 h-5" /> Recent Payments</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {loadingPayments ? (
                    ...
                    <tbody>
                      {(payments as any[]).map((p) => (
                        <tr key={p.id} className="border-b border-border last:border-0 hover:bg-secondary/30 transition-colors">
                          <td className="px-4 py-3 text-muted-foreground">{p.profiles?.email ?? String(p.user_id).slice(0,8)+'…'}</td>
